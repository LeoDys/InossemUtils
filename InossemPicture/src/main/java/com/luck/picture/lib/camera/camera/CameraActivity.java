...
import com.luck.picture.lib.camera.cropper.CropImageView;
import com.luck.picture.lib.camera.cropper.CropListener;
import com.luck.picture.lib.camera.custom.CameraPreview;
import com.luck.picture.lib.camera.global.Constant;
...
public class CameraActivity extends Activity implements View.OnClickListener {
    ...
    private void init() {
        ...
        mType = getIntent().getIntExtra(InossemCustomCamera.TAKE_TYPE, 0);
...
//        LinearLayout.LayoutParams containerParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) width);
...
//        mFlCameraOption.setLayoutParams(cameraOptionParams);
        // TODO 最新修改样式
        float h = (int) (screenMaxSize * 0.75f);
        float w = (int) (screenMinSize * 0.75f);


        float flCameraOptionWidthNew = (screenMinSize - w) / 2;

        // 相机区域
        LinearLayout.LayoutParams containerParamsNew = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) h);
        mLlCameraCropContainer.setLayoutParams(containerParamsNew);

        // 凸显阴影的图片范围
        LinearLayout.LayoutParams cropParamsNew = new LinearLayout.LayoutParams((int) w, (int) h);
        mIvCameraCrop.setLayoutParams(cropParamsNew);

        //获取"相机裁剪区域"的宽度来动态设置底部"操作区域"的宽度，使"相机裁剪区域"居中
        LinearLayout.LayoutParams cameraOptionParamsNew = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) flCameraOptionWidthNew);
        mFlCameraOption.setLayoutParams(cameraOptionParamsNew);

        switch (mType) {
            case InossemCustomCamera.TYPE_IDCARD_FRONT:
//                mIvCameraCrop.setImageResource(R.drawable.camera_idcard_front);
                mIvCameraCrop.setImageResource(R.drawable.camera_portrait);
                break;
            case InossemCustomCamera.TYPE_IDCARD_BACK:
        ...
        mCropImageView.crop(new CropListener() {
            ...
            public void onFinish(Bitmap bitmap) {
                ...
                if (FileUtils.createOrExistsDir(Constant.DIR_ROOT)) {
                    ...
                    String imagePath = "";
                    if (mType == InossemCustomCamera.TYPE_IDCARD_FRONT) {
                        imagePath = buffer.append(Constant.DIR_ROOT).append(Constant.ROOT_DIR_NAME).append(".").append("idCardFrontCrop.jpg").toString();
                    } else if (mType == InossemCustomCamera.TYPE_IDCARD_BACK) {
                        imagePath = buffer.append(Constant.DIR_ROOT).append(Constant.ROOT_DIR_NAME).append(".").append("idCardBackCrop.jpg").toString();
                    }

                    if (ImageUtils.save(bitmap, imagePath, Bitmap.CompressFormat.JPEG)) {
                        Intent intent = new Intent();
                        intent.putExtra(InossemCustomCamera.RESULT_IMAGE_PATH, imagePath);
                        setResult(InossemCustomCamera.RESULT_CODE, intent);
                        finish();
                    }
                }
            }
