...
//                }).launch();
     ...
     * @return 压缩之前的File
     */
    private List<File> getFiles(List<LocalMedia> result) {
        List<File> files = new ArrayList<>();
        for (LocalMedia media : result) {
            files.add(new File(media.isCut() ? media.getCutPath() : media.getPath()));
        }
        return files;
    }
     ...
     * @return 压缩完成的file集合
     */
    private List<File> getCompressedFiles(List<String> paths) {
        List<File> files = new ArrayList<>();
     ...
     * @param files
     */
    private void tinyHandleCompressCallBack(List<LocalMedia> images, List<String> files) {
        if (files.size() == images.size()) {
            for (int i = 0, j = images.size(); i < j; i++) {
                // 压缩成功后的地址
