 ...
 * </pre>
 */
public final class ToastUtil_BlankJ {
    ...
    private static void show(final int resId, final int duration) {
        try {
            CharSequence text = Utils_BlankJ.getApp().getResources().getText(resId);
            show(text, duration);
    ...
    private static void show(final CharSequence text, final int duration) {
        Utils_BlankJ.runOnUiThread(new Runnable() {
            @SuppressLint("ShowToast")
            @Override
            public void run() {
                cancel();
                iToast = ToastFactory.makeToast(Utils_BlankJ.getApp(), text, duration);
    ...
    private static void show(final View view, final int duration) {
        Utils_BlankJ.runOnUiThread(new Runnable() {
            ...
            public void run() {
                cancel();
                iToast = ToastFactory.newToast(Utils_BlankJ.getApp());
                iToast.setView(view);
    ...
    private static View getView(@LayoutRes final int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) Utils_BlankJ.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        ...
        static class SafeHandler extends Handler {
            ...
            public void dispatchMessage(Message msg) {
                try {
                    ...
                    Log.e("ToastUtil_BlankJ", e.toString());
                }
            }
        }
    }
    ...
    static class ToastWithoutNotification extends AbsToast {
        ...
        private static final Utils_BlankJ.OnActivityDestroyedListener LISTENER =
                new Utils_BlankJ.OnActivityDestroyedListener() {
                    @Override
        ...
        @Override
        public void show() {
            Utils_BlankJ.runOnUiThreadDelayed(new Runnable() {
                @Override
                public void run() {
                    realShow();
                }
        ...
        private void realShow() {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N_MR1) {
                ...
                Context topActivityOrApp = Utils_BlankJ.getTopActivityOrApp();
                if (!(topActivityOrApp instanceof Activity)) {
                    Log.e("ToastUtil_BlankJ", "Couldn't get top Activity.");
                    return;
                }
                Activity topActivity = (Activity) topActivityOrApp;
                if (topActivity.isFinishing() || topActivity.isDestroyed()) {
                    Log.e("ToastUtil_BlankJ", topActivity + " is useless");
                    return;
                }
                mWM = topActivity.getWindowManager();
                mParams.type = WindowManager.LayoutParams.LAST_APPLICATION_WINDOW;
                Utils_BlankJ.getActivityLifecycle().addOnActivityDestroyedListener(topActivity, LISTENER);
            }
            ...
            mParams.packageName = Utils_BlankJ.getApp().getPackageName();
